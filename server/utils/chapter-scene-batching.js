...
export async function extractChapterScenesInBatches({
  ...
  for (let index = 0; index < batches.length; index++) {
    const batchTitle = `${chapterTitle}（${index + 1}/${batches.length}）`;
    const batchText = batches[index];
    const batchSceneCount = batchSceneCounts[index] || 1;
    onProgressLog?.(`[LLM] 章节「${chapterTitle}」拆分提炼第 ${index + 1}/${batches.length} 段，发送 ${batchText.length} 字，提炼 ${batchSceneCount} 个场景...`);
